...
for index = reshape(selectedIndices, 1, [])
    ...
    if strcmp(series(index).kind, 'bold')
        ...
        if convertedVolumes ~= series(index).fileCount
            warning('DICOM2BIDS:BoldVolumeMismatch', ...
                ['BOLD volume mismatch for %s: %d DICOM files but %d NIfTI ', ...
                 'volumes; continuing with the converted NIfTI.'], ...
                series(index).path, series(index).fileCount, convertedVolumes);
        end
    end
end
...
%VALIDATECONVERTEDFIELDMAPPAIRS Check geometry and report non-opposing PE.
...
for pairIndex = 1:numel(fmapPairs)
    ...
    if ~areOppositePhaseEncoding( ...
            apJson.PhaseEncodingDirection, paJson.PhaseEncodingDirection)
        warning('DICOM2BIDS:NonOpposingPhaseEncoding', ...
            ['Fieldmap run %d does not have opposing phase-encoding ', ...
             'directions; continuing with the converted NIfTI and JSON.'], ...
            fmapPairs(pairIndex).run);
    end
