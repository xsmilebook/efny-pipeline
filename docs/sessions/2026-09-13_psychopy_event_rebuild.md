...
- 多份同任务 CSV 仍按文件名时间选择最新记录；时间解析同时接受有毫秒和无毫秒的
  PsychoPy 文件名。
- `run_dicom2bids_checked.m` 新增 `operation` 开关。`"convert"` 执行完整 DICOM 转换；
  `"events"` 先验证每个 BIDS 被试到 raw 被试目录的一一映射，再调用主函数删除本流程
  拥有的 SST、nback、switch task-level events TSV，并仅对 BIDS 中实际存在的任务
  BOLD 重建 events。影像和其他任务 events 不受影响。
- 最终运行时仅依赖 `dicom2bids_checked.m` 和 `run_dicom2bids_checked.m`，便于在不同
  转换环境中拷贝使用。

## 验证

MATLAB R2025a 的整合测试覆盖标准 SST/nback/switch 文件名、混合文件列表、带额外
`SST_0702` token 的文件、无毫秒时间戳、多任务歧义文件名、旧 events 替换及其他任务
events 保留。
